...
switch ($_GET['action']){
 case 'getsetting':
     ...
     while ($result = DB :: fetch ($query)){
         ...
         if (mb_strlen(strip_tags(trim($result['content']))) > 50){
             $result['content'] = mb_substr(strip_tags(trim($result['content'])), 0, 50) . '...';
             }
     ...
     $data ['setting'] = $setting?$setting:array(
        "page_switch" => 1, "footer_js_switch" => 1, "footer_text_switch" => 1, "bg_switch" => 0, "page_footer_js" => "", "page_footer_text" => "", "bg_images" => ""
        );
     break;
 case 'savesetting':
     $zw_custompage -> saveSetting('setting', json_encode(array(
                'page_switch' => $_POST['page_switch'] == 1?1:0,
