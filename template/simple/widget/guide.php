...
<button class="btn submit" onclick="$('#guide_page_1').hide();$('#guide_page_2').show();">下一步 &raquo;</button>
</div>
<div id="guide_page_2" class="hidden">
<p>首先，你需要绑定你的百度帐号。</p><br>
<p>为了确保帐号安全，我们只储存你的百度 Cookie，不会保存你的帐号密码信息。</p>
<p>你可以通过修改密码的方式来让这些 Cookie 失效。</p><br>
<form method="post" action="api.php?action=baidu_login" target="_blank">
<p>百度通行证：<input type="text" name="username" placeholder="推荐使用邮箱登录" required value="" /></p>
<p>通行证密码：<input type="password" name="password" placeholder="百度通行证密码" required value="" /></p>
<p><input type="submit" value="绑定百度帐号" /> <a href="<?php echo cloud::get_api_path(); ?>manual_bind.php?sid=<?php echo cloud::id(); ?>&formhash=<?php echo $formhash; ?>" class="btn" target="_blank">手动绑定</a></p>
</form>
</div>
<div id="guide_page_manual" class="hidden"></div>
<div id="guide_page_3" class="hidden">
<p>一切准备就绪~</p><br>
<p>我们已经成功接收到你百度帐号信息，自动签到已经准备就绪。</p>
