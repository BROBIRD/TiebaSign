...
function load_user_info(){
	...
	$.getJSON("ajax.php?v=get-bind-status", function(result){
		if(!result) return;
		$('#content-user_info .tab').addClass('hidden');
		if(result.no == 0){
			$('#content-user_info .tab-binded').removeClass('hidden');
			...
			$('#content-user_info .tab-bind').removeClass('hidden');
		}
...
function parse_hash(){
	...
	if(hash == "guide"){
		$('#menu_guide').click();
	}else if(hash == "user_info"){
		guide_viewed = true;
		$('#menu_user_info').click();
